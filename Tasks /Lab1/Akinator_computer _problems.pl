...
% Доп. вопросы.
 problem(blue_screen) :-
    query('Does the computer show a blue screen error').

problem(noisy_fan) :-
    query('Is there a noisy fan during operation').
...
% Новые объекты без добавления доп. вопросов.
fault(cpu_failure) :-
    problem(long_beep),
    problem(short_beep),
    problem(blank_display).

fault(cpu_overheating) :-
    problem(continuous_beeps),
    problem(blank_display).

% Новые объекты с добавлением доп. вопросов.
fault(virus) :-
    problem(repeating_short_beeps),
    problem(blank_display),
    problem(noisy_fan).

fault(hardware_failure) :-
    problem(repeating_short_beeps),
    problem(continuous_beeps),
    problem(blue_screen).
